...
function render(){queued=false;if(reduced.matches){runner.style.opacity='0';copy.style.opacity='1';copy.style.transform='none';return}const rect=journey.getBoundingClientRect(),s=scene.getBoundingClientRect();const travel=journey.offsetHeight-scene.offsetHeight;const progress=clamp(-rect.top/travel);const scale=Math.max(s.width/1536,s.height/1024),iw=1536*scale,ih=1024*scale;const ox=(s.width-iw)/2,oy=(s.height-ih)/2;const x=s.width*(.08+1.02*progress);const y=s.top+oy+ridgeY((x-ox)/iw)*ih;runner.style.transform=`translate3d(${x}px,${y}px,0) translate(-50%,-100%)`;runner.style.opacity=String(rect.top>50||progress>=1?0:1);const frame=Math.floor(Math.max(0,-rect.top)/13)%6;runner.style.backgroundPosition=`${frame*20}% 50%`;copy.style.opacity=String(1-clamp(progress*1.8));copy.style.transform=`translateY(${-progress*65}px)`}
